...
('Fakeblock',	'Irvine',	'Mobile',	2,	3,	NULL,	2013,	'Seed',	'George Michael',	'OCs Most Popular Digital Wood Instrument'),
...
('2Smart2Host',	'Brooklyn',	'Data Analytics',	4,	380000,		2014, NULL,	'Acquired',	'Evan Nails',	'Dynamic Pricing for Vacation Rentals'),
...
('Virtual Tour', 'Chicago',	'E-commerce',	12,	750000,	120000000,	2014,	'Seed',	'Ari LBoy',	'Real Estate for Fake People'),
...
('OrangeYouLonely',	'Minneapolis',	'Social',	20,	1200000, 1400000,	2012,	'Seed',	'Gus Dawson', 'Dating App for Farmers'),
...
--How many columns are there? 70
-- SELECT *
-- FROM startups;

--Calculate the total number of companies in the table.

SELECT COUNT (name) AS number_of_companyies
FROM startups;

--We want to know the total value of all companies in this table.
SELECT SUM(valuation) AS total_valuations
FROM startups;


--What is the highest amount raised by a startup?
SELECT MAX(raised) AS highest_amout_raised
FROM startups;

--Edit the query so that it returns the maximum amount of money raised, during ‘Seed’ stage.
SELECT MAX(raised) AS highest_amout_raised_seed_stage
FROM startups
WHERE stage = 'Seed';

--In what year was the oldest company on the list founded?
SELECT MIN(founded) AS oldest_startup
FROM startups;
...
SELECT AVG(valuation), category AS average_valuation_by_category
FROM startups
GROUP BY category;
...
SELECT ROUND(AVG(valuation), 2), category AS rounded_average_valuation_by_category
FROM startups
GROUP BY category;
...
SELECT ROUND(AVG(valuation), 2), category AS rounded_average_valuation_category_Highest_first
FROM startups
GROUP BY category
ORDER BY rounded_average_valuation_category_Highest_first DESC;
...
SELECT location, AVG(employees) AS average_employee_by_location
FROM startups
GROUP BY location;
